[![Build Status](https://dev.azure.com/tysonandre775/phan/_apis/build/status/phan.phan?branchName=v6)](https://dev.azure.com/tysonandre775/phan/_build/latest?definitionId=3&branchName=v6)
...
The type info for these come from a big file called [FunctionSignatureMap. (and its deltas)](https://github.com/phan/phan/tree/v6/src/Phan/Language/Internal).
...
See [the Contribution Guidelines](https://github.com/phan/phan/blob/v6/.github/CONTRIBUTING.md) for some guidance on making great issues and pull requests.
...
An [FQSEN](https://github.com/phan/phan/blob/v6/src/Phan/Language/FQSEN.php) is a Fully Qualified Structural Element Name.
...
An FQSEN for an element will inherit from the abstract class [\Phan\Language\FQSEN](https://github.com/phan/phan/blob/v6/src/Phan/Language/FQSEN.php). The actual FQSEN for each element (classes, methods, constants, properties, functions) will be defined by classes in the [\Phan\Language\FQSEN](https://github.com/phan/phan/tree/v6/src/Phan/Language/FQSEN) namespace.

### Type and UnionType

A [Type](https://github.com/phan/phan/blob/v6/src/Phan/Language/Type.php) is what you'd expect and can be a native type like `int`, `float`, `string`, `bool`, `array` or a non-native type for a class such as `\Phan\Language\Type`. Types can also be a generic array such as `int[]`, `string[]`, `\Phan\Language\Type[]`, etc. which denote an array of type `int`, `string` and `\Phan\Language\Type` respectively.

A [UnionType](https://github.com/phan/phan/blob/v6/src/Phan/Language/UnionType.php) denotes a set of types for which an element can be any of them. A UnionType could be something like `int|string` to denote that something can be an `int` or a `string`.  See [[About Union Types]] for more details.
...
The [CodeBase](https://github.com/phan/phan/blob/v6/src/Phan/CodeBase.php) in Phan is an object that maps FQSENs to a representation of the object for both scanned code and internal PHP elements.

Classes, for instance are stored and looked up from the [Class Map](https://github.com/phan/phan/blob/v6/src/Phan/CodeBase/ClassMap.php) whereby a class can be fetched via the class `getClassByFQSEN`.
...
The [Context](https://github.com/phan/phan/blob/v6/src/Phan/Language/Context.php) represents the state of the world at any point during parsing or analysis. It stores the following information:
...
The [Scope](https://github.com/phan/phan/blob/v6/src/Phan/Language/Scope.php) maps variable names to [variables](https://github.com/phan/phan/blob/v6/src/Phan/Language/Element/Variable.php) and is housed within the [Context](https://github.com/phan/phan/blob/v6/src/Phan/Language/Context.php). The Scope will be able to map both locally defined variables and globally available variables.

### Structural Elements

The universe of structural element types are defined in the [`\Phan\Language\Element`](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element) namespace and limited to

* [**Clazz**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Clazz.php) is any class, interface or trait. It provides access to constants, properties or methods.
* [**Method**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Method.php) is a method, function or closure.
* [**Parameter**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Parameter.php) is a parameter to a function, method or closure.
* [**Variable**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Variable.php) is any global or local variable.
* [**Constant**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Constant.php) is either a global or class constant.
* [**Comment**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Comment.php) is a representation of a comment in code. These are stored only for classes, functions, methods, constants and properties.
* [**Property**](https://github.com/phan/phan/tree/v6/src/Phan/Language/Element/Property.php) is any property on a class, trait or interface.
...
1. **Parsing**
   All code is parsed in order to build maps from FQSENs to elements (such as classes or methods). During this phase the AST is read for each file and any addressable objects are created and stored in a map within the code base. A good place to start for understanding parsing is [\Phan\Parse\ParseVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Parse/ParseVisitor.php).
...
3. **Analysis**
   ...
   To understand analysis, take a look at [\Phan\Analysis\PreOrderAnalysisVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Analysis/PreOrderAnalysisVisitor.php) and [\Phan\Analysis\PostOrderAnalysisVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Analysis/PostOrderAnalysisVisitor.php).

A great place to start to understand how parsing and analysis happens is in [\Phan\Phan](https://github.com/phan/phan/blob/v6/src/Phan/Phan.php) where each step is explained.

Take a look at the [\Phan\Analysis](https://github.com/phan/phan/tree/v6/src/Phan/Analysis) namespace to see the various bits of analysis being done.
...
* Create a [ParseVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Analysis/ParseVisitor.php) for the root node in [\Phan\Phan::parseNodeInContext](https://github.com/phan/phan/blob/567e427af2f82434b086780fce63c3b8ba48035f/src/Phan/Phan.php#L200-L206)
...
* [\Phan\Analysis\PreOrderAnalysisVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Analysis/PreOrderAnalysisVisitor.php) where we do part of the analysis during the analysis phase.
* [\Phan\Analysis\PostOrderAnalysisVisitor](https://github.com/phan/phan/blob/v6/src/Phan/Analysis/PostOrderAnalysisVisitor.php) where we do another part of the analysis during the analysis phase.
* [\Phan\AST\UnionTypeVisitor](https://github.com/phan/phan/blob/v6/src/Phan/AST/UnionTypeVisitor.php) where we do much of the work figuring out the types of things throughout analysis.
...
- [`Element::VISIT_LOOKUP_TABLE`](https://github.com/phan/phan/blob/v6/src/Phan/AST/Visitor/Element.php) tells you what visitor methods are called for a given `\ast\Node->kind`.
...
A [`\Phan\AST\ContextNode`](https://github.com/phan/phan/blob/v6/src/Phan/AST/ContextNode.php) contains a lot of useful functionality, such as locating the definition(s) of an element from a referencing node (class, function-like, property, etc) in a Context.

### Testing your changes

See [tests/README.md](https://github.com/phan/phan/tree/v6/tests#scripts) for how to run tests and what Phan's various tests do.
